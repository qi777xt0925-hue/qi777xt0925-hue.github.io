...
이 글은 계산기 사이트에 실립니다. 독자는 계산 결과의 근거를 알고 싶어서 들어옵니다.${
    USE_SEARCH
      ? '\n\n4대보험 요율·비과세 한도처럼 시스템 프롬프트에 이미 적힌 값은 검색하지 말고 그대로 쓰세요. 검색은 거기 없는 것(제도 절차, 신청 기한, 서류명, 최근 개정)에만 쓰고, 출처로 쓸 공식 자료 주소를 확보하는 데 쓰세요. 검색해도 확실하지 않으면 수치를 쓰지 말고 확인 방법을 안내하세요.'
      : ''
  }`;
