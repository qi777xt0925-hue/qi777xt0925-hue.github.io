/* ─────────────────────────────────────────────────────────────
   ...
   기준금리가 바뀌면 아래 baseRate 만 고치면 페이지 전체가 따라갑니다.
   한국은행 기준금리 추이를 확인하고 고치세요. 자주 바뀝니다.
...
const FINANCE = {
  year: 2026,
  baseRate: 3.0,        // 한국은행 기준금리 (%) — 2026-08-27 인상분
  rentSpread: 2.0,      // 주택임대차보호법 시행령 제9조 제2항의 가산 이율 (%)
  rentCeiling: 10.0,    // 같은 시행령 제1항의 연 1할. 둘 중 낮은 쪽이 상한입니다
  pyeongToM2: 400 / 121 // 1평 = 3.3058㎡ (6자 × 6자)
};

/**
 * 법정 전월세 전환율 상한 (주택임대차보호법 제7조의2)
 *
 * 조문은 두 비율 중 "낮은" 쪽을 상한으로 정합니다.
 *   1호 — 시행령이 정한 비율 (연 1할)
 *   2호 — 한국은행 기준금리 + 시행령이 정한 이율 (2%)
 * 기준금리가 8%를 넘지 않는 한 2호가 낮아 그쪽이 걸립니다.
 */
function legalConversionRate(){
  return Math.min(FINANCE.baseRate + FINANCE.rentSpread, FINANCE.rentCeiling);
}
